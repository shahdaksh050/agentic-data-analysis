...
## 4. FULL ROADMAP to "best version" (assessed 2026-06-11)

### Tier 1 — the data-science brain (MILESTONE A — in progress this session)
| # | Item | Detail |
|---|------|--------|
| 1 | **Clustering tool** | `cluster_data`: KMeans with auto-k via silhouette (k=2..8), per-cluster profiles, PCA 2-D coords for the dashboard, saved model. Fills the promised-but-missing "blank target = clustering" path. Registered in ToolRegistry + prompt rules + deterministic fallback + dashboard scatter. |
| 4 | **Auto-treatment of profiler warnings** | Training pipeline *acts* on diagnoses: drop identifier columns, log1p severely skewed non-negative features, `class_weight="balanced"` / `scale_pos_weight` on imbalanced targets. Treatments reported in tool output → report. Must be deterministic so train/evaluate/viz see identical features. |
| 5 | **Tuning + explainability** | Light `RandomizedSearchCV` (n_iter≈8, cv=3, seeded, only when rows ≤ 20k) for RF/XGB/LogReg with `best_params` reported. Permutation importance on the held-out split in `evaluate_model` → top-5 drivers with direction + plain-language narrative sentences. |
| 7 | **Self-contained HTML report** | `report.html` with embedded interactive Vega-Lite charts (vega-embed CDN), objective-driven executive summary, insights, drivers, metrics, tool log. Generated by controller after Stage 7; downloadable from UI. |
| 12 | **Prompt-injection hardening** | Dataset-derived strings (column names, category values) are sanitised (`sanitize_for_prompt`: strip control chars/newlines, clip length) before entering any LLM prompt — in `DatasetMetadata.to_prompt_string` and profiler summaries. |
| 13 | **CSV formula-injection escaping** | `escape_csv_formulas(df)` helper prefixes `=`, `+`, `-`, `@` cells with `'` for any user-facing CSV export path. |

### Tier 2 — product + thesis evidence (MILESTONE B — next session)
| # | Item | Detail |
|---|------|--------|
| 8 | **"Ask your data" tab** | Post-analysis NL chat: LLM answers from memory/tool results; may plan one extra tool call per question. Highest demo value. |
| 9 | **What-if prediction tab** | Form built from feature schema → load best `.pkl` → live prediction + probability. Makes the trained model usable. |
| 6 | **Benchmark harness** | `scripts/benchmark.py`: run agent over 4-5 public datasets (Titanic, Wine, Adult, California housing), emit agent-vs-baseline metrics table. The README promises this; examiners look for it. |
| 2 | **Objective intent parser (non-LLM)** | Keyword/rule classifier (predict/drivers/segment/trend/compare) so the deterministic fallback plan is objective-aware too. |
| 3 | **Time-series tool** | seasonal decomposition + naive/ARIMA comparison when profiler finds a datetime axis. |
| 10 | **Run history** | SQLite store (dataset hash, objective, report, dashboard) + "previous runs" sidebar. |

### Tier 3 — platform hardening (MILESTONE C)
| # | Item | Detail |
|---|------|--------|
| 11 | **Broader ingestion** | Delimiter/encoding sniffing (charset-normalizer), JSON/Parquet, row-sampling for >500k rows. |
| 14 | **Service layer + deployment** | FastAPI (`POST /analyses`, `GET /analyses/{id}`), Dockerfile, LLM retry-with-backoff, token/cost accounting. |
| 15 | **CI depth** | Coverage threshold, `pip-audit` + `bandit` jobs. |
| — | Stretch | Multi-dataset joins, NLP text columns, local LLM (Ollama), auth/rate limiting, PDF export, Plotly toggle. |
